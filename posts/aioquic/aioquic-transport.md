...
- AIOQUIC 生成的用于 wireshark 抓包解密的 secrets_log_file 文件格式有问题，需要进行如下调整：
  - 找到 aioquic 安装包中的 `aioquic/quic/connecion.py` 文件。
  - 修改 SECRETS_LABELS。
    - 原 SECRETS_LABELS
...
因此，如果有 N 个 scid 和 M 个 dcid，有 N*M 个的二元组 (scid, dcid) 指向了同一个连接。这和 TCP 通过四元组标识连接有本质的不同。

这样设计的初衷是为了保障在连接迁移时的隐私性，QUIC IETF 建议在进行连接迁移时，使用另一个可用的 dcid 进行请求。

IETF 原文对连接迁移时使用新的连接 ID 的建议：
...
一个 UDP 报文中会包含至少一个 Packet，而一个 Packet 中至少包含一个 Frame，Frame 可以属于不同的 Stream：
...
Frame 区分了不同的数据类型，例如传输数据的 STREAM Frame，流控的 MAX_DATA Frame。为什么需要区分 Packet，用 Packet 来聚合 Frame 呢？
...
- Long Header Packet 的通用 Header 长度大小范围为：7 - 47 个字节。
...
除了版本协商和 Retry 这两个特殊的 Packet，其他 Packet 都有一个 Packet Number，用于标识发送的 Packet，即便数据重发也会额外再使用一个 Packet Number。
...
AIOQUIC 实现时划分了 Packet 空间，但是 Packet Number 并没有隔离，因此 AIOQUIC 中的 Packet Number 在所有空间中都是唯一的。
...
在 AIOQUIC 的实现中，最大报文大小限制在了 1200 字节：

```py
# AIOQUIC 中，Packet Max Size 就是指的 UDP 最大报文大小
...
AIOQCUI 的工作机制核心是如何处理连接、接收数据、发送数据以及超时，下图提供了一个整体的工作流程概览：
...
需要注意，Client 发起连接的时候一定要填充 Initial Packet 至少 1200 字节，这是因为往往服务器包含证书信息，要返回给 Client 的数据量较大，而此时又因为没有完成地址验证（除非开启了 Retry 地址验证），所以会导致 Server 无法返回足够的握手数据给 Client，这将会导致无法握手成功。
...
- AIOQUIC 保障 SCID 和 DCID 都有 8 个，因此双方会互发 New Connection ID 帧 7 个。Initial Packet 已经各生产了 1 个，所以这样一共有 8 个。

连接握手 TLS 以及 0-RTT 握手更详细的信息可以参考 [AIOQUIC-TLS](aioquic-tls.md)。

### 0-RTT 连接握手

QUIC 如果在 1-RTT 建立连接是，使用了 psk 则 Server 会下发 NewSessionTicket，旨在下次 AIOQUIC 发起连接建立时可以支持 0-RTT。
...
- 发送 STREMA 帧时，在 STREAM 帧中添加 FIN 标识。
- 发送 RESET STREAM 帧，由发送方终止 Stream 上数据的发送。
- 发送 STOP STREAM 帧，由接收方终止 Stream 上数据的接收。

AIOQUIC 的处理：

- AIOQUIC 在处理 STREAM FIN 时，会自动记录 STREAM 的结尾符。
- AIOQUIC 在处理 RESET STREAM 帧时，向业务层传递 RESET 的事件，交给业务层处理。
- AIOQUIC 在处理 STOP STERAM 帧时，除了记录日志，并不会做其他动作。

AIOQUIC 中，RESET STREAM 帧发送交给应用层负责，当应用层需要立即终止发送时就调用 `reset_stream(stream_id, error_code)` 函数。
...
- Endpoint 发生地址迁移时，又 Peer 通过路径校验来确定连通性。
...
无状态重置（Stateless Rest）是无法访问连接时的最后选择，类似于 TCP 的 RST。

无状态重置用于传递致命的错误。

无状态重置是使用一个特别的 Packet：
...
- AIOQUIC 虽然实现了 reset_token 在传输参数和 NCI 中的传递，但是并没有实现对于无状态重置的处理。
...
Initial(trans_parameter)    -------->                   [Know Endpoint MAX_DATA / MAX_STREAM / MAX_STRAM_DATA]
[Know Peer MAX_DATA / MAX_STREAM / MAX_STRAM_DATA]      <--------- Initial(trans_parameter) & Handshake
Packet  ---------------------------->
                                                        <--------- Packet
...
这和其他可靠传输协议有所区别，TCP、KCP 的实现中，拥塞窗口可能减半（快速重传导致），也可能直接减到 1（丢包导致）。
...
- PTO 无法进行丢失检测，减少了重传的速率，可能导致较大的延时。这不是 AIOQUIC 的问题，是 QUIC 协议本身的要求。
- 出现丢包时，即便后续网络稳定也无法重新进入慢启动阶段。这不是 AIOQUIC 的问题，是 QUIC 协议本身的要求。
