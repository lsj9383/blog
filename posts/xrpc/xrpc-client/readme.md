...
- [XRPC Client](#xrpc-client)
    ...
    - [Sequence Diagram](#sequence-diagram)
        - [Client Send Request & Receive Response](#client-send-request--receive-response)
...
Xrpc Client 封装了请求 Server 的接口，这些 Server 包括 XRPC Server，Redis Server，HTTP Server 等等。

通过提供 ServiceProxy 的方式，为应用开发提供了访问这些 Server 的接口和能力，ServiceProxy 可以简单的理解为是应用访问 Server 的代理，这些代理封装了相关的网络处理。
...
participant xrpc_client as XrpcClient
participant concrete_service_proxy as ConcreteServiceProxy
participant service_proxy as ServiceProxy
participant future_transport as FutureTransport

main ->> xrpc_client: 构造 Xrpc Client
xrpc_client -->> main: 返回 Xrpc Client 实例

main ->> xrpc_client: 通过 GetServiceProxy() 获得对应的 ConcreteServiceProxy

main ->> main: MakeClientContext() 构造上下文

main ->> concrete_service_proxy: 调用数据发送接口
concrete_service_proxy ->> concrete_service_proxy: 配置 context
concrete_service_proxy ->> concrete_service_proxy: CLIENT_PRE_RPC_INVOKE 埋点处理，其中包括路由选择
concrete_service_proxy ->> service_proxy: UnaryInvoke()
service_proxy ->> service_proxy: CLIENT_PRE_SEND_MSG 埋点处理
service_proxy ->> future_transport: SendRecv
future_transport -->> service_proxy: 响应
service_proxy ->> service_proxy: CLIENT_POST_RECV_MSG 埋点处理
service_proxy -->> concrete_service_proxy: 响应
concrete_service_proxy ->> concrete_service_proxy: CLIENT_POST_RPC_INVOKE 埋点处理
concrete_service_proxy -->> main: 拿到响应
```

异步请求处理是类似的，只是调用的接口不同，并返回的是 Future。

ServiceProxy 接收响应事时，相关网络事件回调在 [ProxyOptionToTransInfo](#proxyoptiontotransinfo) 中进行设置。

### Request & Response Object

Xrpc Client 请求和响应涉及到对相关对象的转换和编码，这里做个简单的介绍。
