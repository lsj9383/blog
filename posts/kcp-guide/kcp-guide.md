...
while True:
    # KCP 核心的驱动方法，将会发送 ACK、探测消息、负载数据
...
KCP 是存在连接的概念的，但是 KCP 并不负责创建连接。

KCP 连接即 conv，所有 KCP 传输都需要基于一个 conv 的会话标识，通信的双发需要一致的 conv。
...
KCP 需要通过 update 函数进行驱动，因此 update 的调用时间间隔是非常重要的：

- 时间间隔过长，导致无法及时的进行数据传输。
- 时间间隔过短，导致 CPU 的占用率会增大。

在 KCP 的 Demo 和相关实践中可以看到通常设置的时间间隔是 10 - 100ms。
...
KCP 作者称 `ikcp_check()` 函数会返回正确的 update 调用时机（考虑 interval 和重传时间戳），而 `ikcp_check()` 返回的时间戳会限制为小于 interval，这意味着：

- KCP 的 check 保障在一个 interval 内，至少需要进行一次 update。

即便 KCP 没有任何输入和任何输出，KCP 的 check 也应该保障 interval 中至少调用一次，这是为什么呢？因为需要探测。

- KCP 没有任何输入和输出，但是对端窗口接收窗口大小先前协商为 0 了，这会导致下次有数据需要发送时，无法立即发送，需要等待探测完毕。
...
- 发生了 IO 事件，需要在下一个 interval 立即处理，不能等到下一次 check 的时间点。
...
可以通过任意链表中的节点进行链表的遍历。在实际使用时，链表需要有一个入口节点。
